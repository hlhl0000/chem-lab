...
const GAS = {
  ...
  /* ★ SUBSTEPS 2 → 4 (2026-08-17 · 샤를 법칙에서 온도에 따른 속도 변화를 더 보이게).
     ⚠ 입자 속도(v = √(2T/M))를 손대지 않았다 — 손대면 압력 P ∝ v² 가 함께 커져
       보일·샤를이 깨진다. 대신 **한 프레임에 물리를 두 번 더 돌려** 화면 속 시간을
       2배로 흘린다: 온도 대비는 √T 그대로, 프레임당 이동량만 2배가 된다.
       (180 K 1.90 → 3.79 px/f · 600 K 3.46 → 6.93 px/f)
     검산(12회 반복 × 4000프레임): 압력 1.0085 → 1.0132 atm, t = 0.36 으로 편향 없음.
       부피·PV/NkT·샤를 V∝T 는 완전 불변(V/T = 214.290 고정).
     아래 압력 표본 게이트(tAcc > 4.0)를 함께 올려야 표본 시간이 유지된다 — 안 올리면
       표본이 절반이 되어 압력 표시가 더 흔들린다. */
  DT: 0.05, SUBSTEPS: 4,
...
function makeGas() {
  ...
  function frame(movePiston) {
    ...
    thermostat();
    if (st.tAcc > 4.0) {   // SUBSTEPS 4와 짝 — 압력 표본 시간을 종전과 같게 유지한다
