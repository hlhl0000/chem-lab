...
/* ==== CORE-BEGIN ====
  ...
  var P = {
    ...
    K: 12,
    /* 폭풍 일정 — 3세대로 압축 (2026-08-24 사용자 지시: 4라운드 → 3라운드).
       6세대판 → _설계보정_anolis_20260822/anolis_core_6gen_backup.js
       4세대판 → _설계보정_anolis_20260822/anolis_core_4gen_backup.js
       ★ 4세대판 TH 의 «앞 세 개를 그대로» 썼다. 그래서 1~3세대 동역학은 4세대판과
         완전히 동일하고 4세대만 사라진다 — 바뀐 수치가 어디서 왔는지 추적할 수 있다.
       ★ 최대 도약은 2→3번째(+0.11)다. 마지막 폭풍이 «바뀐 환경»이다
         (4세대판에서는 바뀐 환경을 두 세대 겪었다 — 압축의 대가). */
    TH: [0.95, 1.03, 1.14],
    GENS: 3, LIFE: 4, MUL: 1.5,
...
/* ★ 2026-08-24 — 폭풍 4회 → 3회 (사용자 지시)로 아래 다섯 값이 바뀌었다.
   4세대판 값은 _설계보정_anolis_20260822/anolis_core_4gen_backup.js 와 함께 검증보고 v5 에 남겼다.
   ⚠ zeroVarNaturalDead 가 41.8% → 4.25% 로 떨어졌다 — 「변이가 없으면 소멸한다」가
     3세대에서는 거의 나타나지 않는다. 교육적 강도 판단은 교사 몫(매뉴얼 P7). */
var VERDICT = {
  zeroVarLamarckEnd: 1.113,   /* 「변이 없는 개체군」 재생(seed 20260822) 용불용설 «물려받은 값» 최종 */
  zeroVarNaturalDead: 4.25,   /* 같은 조건 자연선택 소멸률 (800회) */
  deadLamarck: 1.0,           /* 통상 조건 용불용설 전멸률 (800회) */
  deadNatural: 3.5,           /* 통상 조건 자연선택 전멸률 (800회) */
  zeroVarNaturalMean: 1.030,  /* 변이 없는 개체군의 자연선택 «물려받은 값» — 이탈 정확히 0 */
  zeroVarNaturalBody: 1.056,  /* 같은 조건 자연선택 «몸값» 종점 — 몸은 넓어졌다는 증거 (재생 시드) */
